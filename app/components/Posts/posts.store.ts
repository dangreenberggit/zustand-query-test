...
export const useSelectedPostsIds = () =>
    usePostsStore((state) => state.selectedPostsIds);
// Hook to get actions to work with state
export const usePostsStoreActions = () =>
    usePostsStore((state) => state.actions);
// Hook to check if a post with the provided ID is already selected
export const usePostsIsSelected = (id: number): boolean =>
    usePostsStore((state) => state.selectedPostsIds.has(id));
// Get the amount of selected posts
export const useSelectedPostsCount = (): number =>
    usePostsStore((state) => state.selectedPostsIds.size);
